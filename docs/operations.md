...
`zenith-sim-scheduler-lab` is centered on simulations work in C#. The technical target is: Create a C# reference implementation for scheduler workflows, centered on constraint solving, bounded scenario files, and conflict explanations.
...
The score is deterministic. The current threshold is 178. Risk has a penalty of 4, latency has a penalty of 3, and weight contributes a bonus of 2.
